...
# Taggerist v34.18: fixes + features — (a) CI date-symbol buttons now use contrast-aware text (fixes invisible label when taglist background is yellow); (b) CI symbols are no longer hard-coded: all matcher regexes (_ci_alt) are built from the configured TAGLIST_IDs, so the new [RW] list's symbol replaces others correctly and repeated presses never duplicate; (c) PROC list now reliably refreshes after PROCESS (refresh moved outside the try block; the just-processed file is scrolled to center of PROC); (d) NEW [Default Sort All] button right-justified above UNPROCESSED sorts both lists by Modified date, oldest first; (e) ENTER in SearchBox accepts the first result; (f) focus returns to SearchBox after any button action (tag toggle, CI buttons, Clear/Reduce, PROCESS, Default Sort All)
...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v34.18-buglog.txt', 'a') as f:
                f.write("--- Taggerist v34.18 startup ---\n")
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.18 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.18")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.18-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.18-buglog.txt', 'a') as debug_file:
