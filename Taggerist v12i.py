...
# Taggerist v12i: TF columns 80/10/10/100px; sort toggle; single highlight; TL no lines/spacing + tag highlighting
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v12i - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v12i")
...
class TaggeristList(QFrame):
    ...
    def setup_ui(self):
        ...
        self.copyright_radio = QRadioButton("(c)")
    ...
    def setup_taglist_window(self, layout):
        ...
        self.scroll_area.setStyleSheet("background-color: #000; border: 1px solid #FFFFCC;")
        ...
        self.grid_layout.setVerticalSpacing(self.taglist_vertical_spacing)
...
class TaggeristFiles(QFrame):
    ...
    def setup_ui(self):
        ...
        QTimer.singleShot(200, self.resize_tables)
    ...
    def resize_tables(self):
        for table in [self.proc_table, self.unproc_table]:
            ...
            table.setColumnWidth(0, int(viewport_width * 0.8))
            table.setColumnWidth(1, int(viewport_width * 0.1))
            table.setColumnWidth(2, int(viewport_width * 0.1))
            table.setColumnWidth(3, 100)

    def sort_table(self, table, index):
        if table.horizontalHeader().sortIndicatorSection() == index:
            order = Qt.DescendingOrder if table.horizontalHeader().sortIndicatorOrder() == Qt.AscendingOrder else Qt.AscendingOrder
        else:
    ...
    def open_directory(self, dir_type):
        if dir_type == "proc":
            caption = "Select PROC Directory"
            start_dir = self.proc_dir
        elif dir_type == "unproc":
            caption = "Select UNPROC Directory"
            start_dir = self.unproc_dir
        else:
            return
        new_dir = QFileDialog.getExistingDirectory(self, caption, start_dir)
        if new_dir:
            if dir_type == "proc":
                self.proc_dir = new_dir
                self.proc_path_label.setText(new_dir)
            else:
                self.unproc_dir = new_dir
                self.unproc_path_label.setText(new_dir)
            self.parent.config.set('DIRECTORIES', 'PROC_DIR' if dir_type == "proc" else 'UNPROC_DIR', new_dir)
            self.refresh_lists()
            self.parent.save_settings()
    ...
    def load_file_from_proc(self, item):
        row = item.row()
        filename = self.proc_table.item(row, 0).text()
    ...
    def load_file_from_unproc(self, item):
        row = item.row()
        filename = self.unproc_table.item(row, 0).text()
    ...
    def clear_all_highlights(self):
        ...
        self.unproc_table.clearSelection()
    ...
    def refresh_proc_list(self):
        ...
        if os.path.exists(self.proc_dir):
            ...
            for i, filename in enumerate(files):
                ...
                self.proc_table.setItem(i, 1, QTableWidgetItem(size))
                self.proc_table.setItem(i, 2, QTableWidgetItem(file_type))
                self.proc_table.setItem(i, 3, QTableWidgetItem(mod_time))
            if self.current_file_in_proc:
                for i in range(self.proc_table.rowCount()):
                    if self.proc_table.item(i, 0).text() == os.path.basename(self.current_file_in_proc):
    ...
    def refresh_unproc_list(self):
        ...
        if os.path.exists(self.unproc_dir):
            ...
            for i, filename in enumerate(files):
                ...
                self.unproc_table.setItem(i, 1, QTableWidgetItem(size))
                self.unproc_table.setItem(i, 2, QTableWidgetItem(file_type))
                self.unproc_table.setItem(i, 3, QTableWidgetItem(mod_time))
            if self.current_file_in_unproc:
                for i in range(self.unproc_table.rowCount()):
                    if self.unproc_table.item(i, 0).text() == os.path.basename(self.current_file_in_unproc):
