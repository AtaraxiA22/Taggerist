...
# Taggerist v34.15: crash diagnostics — two search-path fixes did not stop the heap corruption, and the v34.14 crash pattern (searches OK, PROCESS, then crash on next search) points at corruption happening during PROCESS/refresh, merely DETECTED at the next search's free(). This version (a) enables faulthandler so the true C-level abort site prints a Python stack trace to stderr, (b) logs PySide6/Python/Qt versions to the buglog at startup, and (c) removes QCoreApplication.processEvents() from the table click handler — a re-entrancy hazard mid-signal. When it crashes, paste the faulthandler stack trace + the version lines from the buglog
...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v34.15-buglog.txt', 'a') as f:
                f.write("--- Taggerist v34.15 startup ---\n")
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.15 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.15")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.15-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.15-buglog.txt', 'a') as debug_file:
