...
# Taggerist v34.13: crash fix — search results QListWidget (rapid clear/repopulate per keystroke caused 'free(): invalid pointer' heap aborts in PySide6) replaced with a flow of ClickableTagLabel chips in a plain QFrame, reusing the stable signal-based label pattern proven since v34.09; per-tag deleteParent(None) teardown on refresh, no item-view churn
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.13 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.13")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.13-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.13-buglog.txt', 'a') as debug_file:
    ...
    def perform_search(self, text):
        ...
        for tag, n in matches[:12]:
            fg, bg = self.parent.get_taglist_colors(n)
            chip = ClickableTagLabel(tag, self.search_results)
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.13-buglog.txt', 'a') as f:
