...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.11 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.11")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.11-buglog.txt', 'a') as dbg:
    ...
    def refresh_taglist_buttons(self):
        ...
        for i, btn in enumerate(buttons, start=1):
            ...
            if i == self.active_taglist_number:
                btn.setStyleSheet(f"color: {f}; background-color: {b}; border: 1px solid {border}; padding: 2px;")
            else:
    ...
    def apply_border_colors(self, color):
        ...
        self.search_results.setStyleSheet(f"background-color: #000; color: #fff; border: 1px solid {color};")
        self.search_edit.setStyleSheet(f"background-color: #000; color: #fff; border: 1px solid {color};")
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.11-buglog.txt', 'a') as debug_file:
        ...
        self.pathname_edit.setFont(QFont("Ubuntu", 18))
        ...
        bottom_layout.addWidget(self.search_edit)
    ...
    def update_length_monitor(self):
        ...
        if length >= 151:
            ...
            bar_color = '#00cc00'
        border = getattr(self, 'active_border_color', '#FFFFCC')
        self.length_bar.setStyleSheet(
            f"QProgressBar {{ background-color: #000; border: 1px solid {border}; }}"
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.11-buglog.txt', 'a') as f:
