...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v35.06-buglog.txt', 'a') as f:
                f.write("--- Taggerist v35.06 startup ---\n")
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v35.06 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v35.06")
...
class TaggeristViewer(QFrame):
    ...
    def setup_ui(self):
        ...
        top_spacer = QWidget()
        top_spacer.setFixedHeight(self.image_info_label.sizeHint().height())
        ...
        for btn in (self.external_btn, self.prev_btn, self.next_btn):
            ...
            btn.setStyleSheet("color: #fff; background-color: #000; border: 2px solid #fff; font-weight: bold; padding: 4px 8px;")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v35.06-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v35.06-buglog.txt', 'a') as debug_file:
        ...
        for btn in (self.x_btn, self.y_btn, self.z_btn):
            btn.setFont(QFont("Ubuntu", 11, QFont.Bold))
            btn.setMinimumHeight(30)
        top_grid.addWidget(self.x_btn, 1, 0)
        top_grid.addWidget(self.y_btn, 2, 0)
        top_grid.addWidget(self.z_btn, 3, 0)
        # Col 1: Clear (row 1), Reduce (row 2), Sort Tagline + Skip (row 3)
        ...
        top_grid.addWidget(self.sort_tags_btn, 3, 1)
        ...
        top_grid.addWidget(self.process_btn, 1, 2, 2, 1)
        ...
        top_grid.addWidget(self.skip_btn, 3, 2)
