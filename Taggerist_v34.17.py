...
# Taggerist v34.17: crash root cause #2 — PySide6 6.11.2 keeps a stale shiboken wrapper when widgets are removed via layout itemAt(...).widget().setParent(None); the wrapper later resolves to the wrong C++ object and deleting it aborts with free(): invalid pointer (a reported PySide6 6.11 bug; Taggerist's _refill_taglist does exactly this on EVERY taglist switch AND window resize — matching every crash since v34.03, with v34.16's buglog showing select_taglist immediately before both aborts). Fix: tag labels are now tracked in self.tag_label_refs; teardown walks that list, removes each from the layout by direct reference, and deletes via deleteLater() — no layout-item lookups on delete paths. Also fixes: date-prefix buttons now strip any existing CI symbol before applying the new one even when no datestamp is present
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QFrame, QVBoxLayout, QHBoxLayout, QWidget,
                               ...
                               QSplitter, QSizePolicy, QTableWidget, QTableWidgetItem, QHeaderView, QDialog, QProgressBar)
...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v34.17-buglog.txt', 'a') as f:
                f.write("--- Taggerist v34.17 startup ---\n")
    ...
    def apply_border_colors(self, color):
        self.setStyleSheet(f"""
            ...
            QProgressBar {{ background-color: #000; border: 1px solid {color}; }}
        """)
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.17 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.17")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.17-buglog.txt', 'a') as dbg:
    ...
    def refresh_taglist_buttons(self):
        ...
        for i, btn in enumerate(buttons, start=1):
            ...
            if i == self.active_taglist_number:
                ...
                btn.setStyleSheet(f"color: {f}; background-color: #000; border: 1px solid {border}; padding: 2px;")

    def apply_border_colors(self, color):
        self.active_border_color = color
        self.filename_edit.setStyleSheet(f"background-color: #000; color: #fff; border: 1px solid {color};")
        self.length_bar.setStyleSheet(f"QProgressBar {{ background-color: #000; border: 1px solid {color}; }} QProgressBar::chunk {{ background-color: #00cc00; }}")
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.17-buglog.txt', 'a') as debug_file:
        ...
        self.skip_btn.setStyleSheet("color: #fff; background-color: #e8752a; border: 1px solid #FFFFCC; padding: 6px 18px;")
        ...
        self.length_bar.setTextVisible(False)
        self.length_bar.setFixedHeight(12)
        self.length_bar.setStyleSheet("QProgressBar { background-color: #000; border: 1px solid #FFFFCC; } QProgressBar::chunk { background-color: #00cc00; }")
        top_grid.addWidget(self.length_bar, 2, 1)
        self.process_btn = QPushButton("[PROCESS]")
        self.process_btn.setFont(QFont("Ubuntu", 18, QFont.Bold))
        self.process_btn.setStyleSheet("color: #fff; background-color: #2040c0; border: 1px solid #FFFFCC; padding: 6px 18px;")
        ...
        self.search_row.addWidget(self.search_edit)
        layout.addLayout(self.search_row, stretch=0)
    ...
    def update_length_monitor(self):
        ...
        self.length_bar.setStyleSheet(
            f"QProgressBar {{ background-color: #000; border: 1px solid #FFFFCC; }}"
    ...
    def perform_search(self, text):
        ...
        for idx, chip in enumerate(self.search_chips):
            if idx < len(matches):
                ...
                chip.setText(tag)
                chip.tag = tag
                chip.setStyleSheet(f"color: {fg}; background-color: {bg}; border: 1px solid #FFFFCC; padding: 1px 4px;")
