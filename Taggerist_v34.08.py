...
# Taggerist v34.08: taglist display fix — quote-aware comment stripping so "#fff"-style colors are no longer mangled (tags loaded but rendered black-on-black); deferred first-fill after show so columns wrap at the real viewport height; refill diagnostics in buglog
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.08 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.08")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.08-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.08-buglog.txt', 'a') as debug_file:
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.08-buglog.txt', 'a') as f:
