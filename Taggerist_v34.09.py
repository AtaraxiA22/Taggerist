...
# Taggerist v34.09: crash fix — tagname click handling moved from per-instance mousePressEvent monkey-patching (a known PySide6 cause of "invalid pointer" heap aborts when labels are destroyed) to a proper ClickableTagLabel subclass with a signal; palette images converted to RGBA before thumbnailing
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.09 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.09")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.09-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.09-buglog.txt', 'a') as debug_file:
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.09-buglog.txt', 'a') as f:
