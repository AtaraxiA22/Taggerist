...
# Taggerist v34.12: UI refinements per v34.11 user feedback — SearchBox moved to line#6 left of results (default font, black background); thermometer given fixed neutral border and brighter green/yellow/red fills; pathname font reduced to default; active taglist button now uses contrast-aware text color (fixes invisible white-on-yellow label); [PROCESS] enlarged bold white-on-blue and [SKIP] enlarged white-on-orange; [Clear]/[Reduce] default size; row-3 [?] removed; FilenameEditBox text flush-right; SearchBox no longer follows taglist border color — thermometer-style filename length bar under FilenameEditBox (green <100, yellow 100-149, red >=150); enlarged pathname/FilenameEditBox fonts (~2x); optional (c)->© conversion via new CI_SYMBOL config ('©' = convert to ©, '(c)' = keep as typed); [Search] box with live filtering across all three taglists (click result to toggle tag); all frame borders now use the active taglist's background color
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QFrame, QVBoxLayout, QHBoxLayout, QWidget,
                               ...
                               QListWidgetItem, QSplitter, QSizePolicy, QTableWidget, QTableWidgetItem, QHeaderView, QDialog, QProgressBar)
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.12 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.12")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.12-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.12-buglog.txt', 'a') as debug_file:
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.12-buglog.txt', 'a') as f:
