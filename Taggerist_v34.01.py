...
# Taggerist v34.01: Wire SKIP/PROCESS/taglist-choice/CI buttons; remove vestigial blink radios; consolidated config
...
class TaggeristMainWindow(QMainWindow):
    ...
    def load_taglist_and_conversions(self):
        ...
        active = getattr(self.tl, 'active_taglist_path', None)
        if active:
            taglist_file = active
        else:
            taglist_file = os.path.expanduser(strip_comment(self.get_config('DIRECTORIES', 'TAGLIST_1')))
            self.tl.active_taglist_path = taglist_file
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.01 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.01")
...
class TaggeristList(QFrame):
    def update_date_prefix(self, symbol):
        ...
        new_filename = re.sub(r'[@©\(c\)]+(?=\d{2,4}\.\d{4}-\d{4}\.)', symbol, current_filename)
        ...
        if not re.search(r'\d{2,4}\.\d{4}-\d{4}\.\d+', new_filename):
            ...
            new_filename = f"{symbol}{stamp}{base}{ext}"
    ...
    def select_taglist(self, number):
        ...
        path, font_color, bg_color = parts[0], parts[1] if len(parts) > 1 else 'white', parts[2] if len(parts) > 2 else 'black'
        if not os.path.exists(path):
            QMessageBox.warning(self, "Taglist not found", f"Taglist file not found:\n{path}")
            return
        self.active_taglist_path = path
        self.parent.load_taglist_and_conversions()
        self.refresh_taglist_buttons(font_color, bg_color)

    def refresh_taglist_buttons(self, font_color, bg_color):
    ...
    def setup_ui(self):
        ...
        with open('Taggerist_v34.01-buglog.txt', 'a') as debug_file:
    ...
    def open_readme(self):
        import subprocess
        readme_path = os.path.expanduser("~/readme.md")
    ...
    def _find_datestamp(self, root):
        return re.search(r'(?P<ci>\(c\)|[@\w])?(?P<ds>\d{2,4}\.\d{4}-\d{4}\.\d+)', root)
    ...
    def reduce_filename_edit(self):
        ...
        if m:
            kept = (m.group('ci') or '') + m.group('ds')
            remainder = root.replace(m.group(0), '', 1)
        else:
            kept = ''
            remainder = root
        out = [kept]
        ...
        if '_wm' in root and '_wm' not in result:
            result += '_wm'
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.01-buglog.txt', 'a') as f:
