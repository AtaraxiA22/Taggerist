...
TAGLIST_ALL = '/home/jack/MEGA/TAGGERIST/TAGGERIST.configs/Taggerist.taglist.[Other].csv,white,red'
...
# Taggerist v33.30: Added [Clr]/[Rdc] logic; restored @/(c) radio + blink timer creation
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v33.30 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v33.30")
...
class TaggeristList(QFrame):
    ...
    def setup_ui(self):
        ...
        with open('Taggerist_v33.30-buglog.txt', 'a') as debug_file:
        ...
        # Add @ and (c) radio buttons (not shown; CI selection uses the [x]/[y]/[z] buttons)
        self.at_radio = QRadioButton("@", self)
        self.copyright_radio = QRadioButton("(c)", self)
        self.at_radio.setStyleSheet("color: #fff; background-color: #000; font-size: 16px;")
        self.copyright_radio.setStyleSheet("color: #fff; background-color: #000; font-size: 16px;")
        ...
        self.at_radio_blink_timer = QTimer(self)
        self.copyright_radio_blink_timer = QTimer(self)
        self.at_radio_blink_timer.timeout.connect(lambda: self.blink_button(self.at_radio))
        self.copyright_radio_blink_timer.timeout.connect(lambda: self.blink_button(self.copyright_radio))
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v33.30-buglog.txt', 'a') as f:
