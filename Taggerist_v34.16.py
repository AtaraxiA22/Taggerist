...
# Taggerist v34.16: crash ROOT CAUSE fixed — faulthandler trace from v34.15 pinned the abort to refresh_proc_list's setRowCount(0): both file tables are rebuilt (setRowCount(0) + insertRow/setItem) with setSortingEnabled(True) active. With sorting on, insertRow/setItem can re-sort mid-fill, so setItem writes into rows that have already moved — corrupting QTableWidget's C++ heap. The corruption was merely DETECTED later at the next mass free (previously the search chips' free — explaining why every search-path fix failed since v34.11). Fix: both refresh functions now disable sorting, clear and refill the table, then re-enable and re-apply the date sort. Version logging + faulthandler kept for confirmation
...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v34.16-buglog.txt', 'a') as f:
                f.write("--- Taggerist v34.16 startup ---\n")
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.16 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.16")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.16-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.16-buglog.txt', 'a') as debug_file:
