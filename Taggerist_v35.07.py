...
class TaggeristMainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            import PySide6
            with open('Taggerist_v35.07-buglog.txt', 'a') as f:
                f.write("--- Taggerist v35.07 startup ---\n")
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v35.07 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v35.07")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v35.07-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v35.07-buglog.txt', 'a') as debug_file:
        ...
        # NEW.1: Col 1, rows 1-3: taglist buttons VERTICAL, one per CI row
        self.abcd_btn = QPushButton()
        self.mnlo_btn = QPushButton()
        self.wxyz_btn = QPushButton()
        for btn in (self.abcd_btn, self.mnlo_btn, self.wxyz_btn):
            btn.setFont(QFont("Ubuntu", 10, QFont.Bold))
            btn.setMinimumHeight(30)
        top_grid.addWidget(self.abcd_btn, 2, 2)
        top_grid.addWidget(self.mnlo_btn, 3, 2)
        top_grid.addWidget(self.wxyz_btn, 4, 2)
        ...
        self.search_edit.setStyleSheet("background-color: #000; color: #fff; border: 1px solid #FFFFCC; font-size: 14px; padding: 6px 4px;")
        self.search_edit.textChanged.connect(self.perform_search)
        self.search_edit.returnPressed.connect(self.accept_first_search_result)
        top_grid.addWidget(self.search_edit, 5, 0, 1, 8)
