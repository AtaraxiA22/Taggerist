...
# Taggerist v34.14: crash fix round 2 — v34.13 proved widget create/destroy churn on the search path is itself the PySide6 heap-corruption trigger (crash persisted with deleteLater chips). Results panel is now a FIXED POOL of 12 pre-allocated ClickableTagLabels created once at startup; searching only updates text/stylesheet/visibility — zero widget lifecycle activity while typing
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.14 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.14")
...
class TaggeristList(QFrame):
    ...
    def select_taglist(self, number):
        ...
        with open('Taggerist_v34.14-buglog.txt', 'a') as dbg:
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.14-buglog.txt', 'a') as debug_file:
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.14-buglog.txt', 'a') as f:
