...
# Taggerist v12j: Fixed TF sort, Process/Skip buttons, and highlighting
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v12j - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v12j")
...
class TaggeristList(QFrame):
    ...
    def setup_ui(self):
        ...
        with open('Taggerist_v33.01-buglog.txt', 'a') as debug_file:
        ...
        # Add radio buttons to the filename layout
        filename_layout.insertWidget(0, self.at_radio)
        filename_layout.insertWidget(1, self.copyright_radio)

        # Connect radio buttons to their logic
        self.at_radio.toggled.connect(lambda: self.update_date_prefix('@'))
        self.copyright_radio.toggled.connect(lambda: self.update_date_prefix('©'))
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v33.01-buglog.txt', 'a') as f:
