...
# Taggerist v34.05: process_filename no longer strips free text (fixed '.JPG'-only filenames); PyQt5 double-binding removed; [?] in row 3; duplicate pathname bar removed; config file preserved
...
class TaggeristMainWindow(QMainWindow):
    ...
    def update_window_title(self):
        if self.current_file:
            self.setWindowTitle(f"Taggerist v34.05 - {os.path.basename(self.current_file)}")
        else:
            self.setWindowTitle("Taggerist v34.05")
...
class TaggeristList(QFrame):
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        with open('Taggerist_v34.05-buglog.txt', 'a') as debug_file:
...
class TaggeristFiles(QFrame):
    ...
    def handle_single_click(self, item):
        try:
            ...
            with open('Taggerist_v34.05-buglog.txt', 'a') as f:
